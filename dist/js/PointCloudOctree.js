...
Potree.PointCloudOctreeProxyNode.prototype = Object.create(
	...
	if (groot != undefined && groot.loaded == true && this.root === undefined) {
		...
		this.traverseBreadthFirst(function(object) {
			if (object instanceof THREE.PointCloud) {
				...
				var boxWorld = Potree.utils.computeTransformedBoundingBox(
					object.boundingBox,
					object.matrixWorld
				);
				var camWorldPos = new THREE.Vector3().setFromMatrixPosition(
					camera.matrixWorld
				);
				var bbWorldPos = object.boundingBox.center();
				bbWorldPos.applyMatrix4(object.matrixWorld);
				var distance = new THREE.Vector3()
					.subVectors(camWorldPos, bbWorldPos)
					.length();
				...
				visible =
					visible &&
					distance < _this.LODDistance / Math.pow(2, object.level);
